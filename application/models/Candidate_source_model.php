...
class Candidate_source_model extends MY_Model
{
    protected $table;
    public function __construct()
    {
        parent::__construct();
        $this->table = $this->TBL_CANDIDATE_SOURCE;
    }


    public function View($source_id = '', $source_name = '', $source_email = '',$is_social_media = '', $is_active = '',  $oder_by_column = 'source_id')
    {
        if ($source_id !== "") {
        ...
        if ($source_email !== "") {
            $this->db->where('source_email', $source_email);
        }
        if ($is_social_media !== "") {
            $this->db->where('is_social_media', $is_social_media);
        }
        if ($is_active !== "") {
    ...
    public function view_count($source_id = '', $source_name = '', $source_email = '',$is_social_media = '', $is_active = '')
    {
        if ($source_id !== "") {
            $this->db->where('source_id', $source_id);
        }
    ...
    public function ViewArray($source_id = '', $job_country = '', $job_code = '', $is_open = '',  $oder_by_column = 'source_id')
    {
        if ($source_id !== "") {
            $this->db->where('source_id', $source_id);
        }
    ...
    public function Update($source_id, $data)
    {
        ...
        if ($this->db->update($this->table, $data)) {
            return $this->db->where('source_id', $source_id)->get($this->table)->result();
        } else {
            return false;
        }
    }
