...
class Advertisement_model extends MY_Model
{
    ...
    {
        parent::__construct();
        $this->table = $this->TBL_ADVERTISEMENT;
    }


    public function View($ad_id = '', $ad_image_path = '', $is_active = '', $oder_by_column = 'ad_id')
    {
        if ($ad_id !== "") {
            $this->db->where('ad_id', $ad_id);
        }
        if ($ad_image_path !== "") {
            $this->db->where('ad_image_path=', $ad_image_path);
        }
    ...
    public function Update($ad_id, $data)
    {
        $this->db->where('ad_id', $ad_id);
        if ($this->db->update($this->table, $data)) {
            return $this->db->where('ad_id', $ad_id)->get($this->table)->result();
        } else {
            return false;
        }
    }
